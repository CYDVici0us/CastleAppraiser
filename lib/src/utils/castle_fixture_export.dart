...
/// Share fixture JSON (and the photo when the castle was captured, not bundled).
class CastleFixtureExport {
  CastleFixtureExport._();

  static Future<void> share(Castle castle) async {
    final hive = castle.hiveCastle;
    final assetName = hive?.debugAssetName;
    final fromAsset = assetName != null && assetName.isNotEmpty;
    final imageFileName = fromAsset
        ? DebugCastleAssets.basename(assetName)
    ...
    final jsonPath = '${directory.path}/${fixture.jsonFileName}';
    ...
    await SharePlus.instance.share(
      ShareParams(
        files: files,
        text: fromAsset
            ? 'Debug castle fixture for ${fixture.image}'
            : 'Debug castle fixture + photo ${fixture.image}',
      ),
    );
  }
